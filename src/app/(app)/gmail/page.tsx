      ...
      <div className="flex-1 overflow-auto">
        <ul className="divide-y divide-slate-100">
          {emails.map((m) => {
            ...
            return (
              <li key={m.id} className="flex items-start gap-3 bg-white px-8 py-4 transition hover:bg-slate-50">
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-red-100 text-xs font-semibold text-red-700">
                  {initials(sender.name)}
                </span>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <p className="truncate font-medium text-slate-900">{sender.name}</p>
          ...
          {emails.length === 0 && (
            <li className="px-8 py-16 text-center text-slate-400">No emails to show.</li>
          )}
        </ul>
      </div>
    </div>
